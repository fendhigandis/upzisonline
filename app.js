...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // -----------------------------------------------------------------
    // --- 🌟 PERBAIKAN FINAL: GEMBOK GANDA TOMBOL LOGIN ---
    // -----------------------------------------------------------------
    const btnSubmit = document.getElementById('btn-auth-submit');
    if (btnSubmit) {
        // Mencegah tombol melakukan submit bawaan HTML
        btnSubmit.type = 'button'; 
        
        // Memindahkan logika login ke fungsi "onclick" tombol
        btnSubmit.onclick = async function(e) {
            ...
            try {
                alert("Sistem mencoba login...\nEmail: " + email); // ALERT PENANDA: Muncul jika file berjalan lancar
                ...
                if (isRegisterMode) {
                    ...
                    // Login berhasil, pop-up akan tertutup dan sistem pindah layar otomatis
                }
            } catch (error) {
                // Menampilkan pop-up error jika ada masalah di Firebase
                alert("GAGAL LOGIN:\nKode: " + error.code + "\nPesan: " + error.message);
            }
        };
    }
...
window.addEventListener('error', function(event) {
    // Jika file gagal dimuat, akan langsung memunculkan pop-up ini
    alert("[INFO ERROR SISTEM]: " + event.message);
});
