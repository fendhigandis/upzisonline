...
enableIndexedDbPersistence(db).catch((err) => {
    console.log('[Sistem] Berjalan di mode memory cache.');
});
...
let profileSettings = {
    ...
    anggota: [],
    logoBase64: 'https://upload.wikimedia.org/wikipedia/commons/thumb/2/24/Nahdlatul_Ulama_Logo.svg/512px-Nahdlatul_Ulama_Logo.svg.png',
...
async function logout() {
    try {
        ...
        profileSettings = { lembaga: '', periode: '', ketua: '', bendahara: '', username: 'bendahara', password: 'admin', relawan: [], anggota: [], logoBase64: 'https://upload.wikimedia.org/wikipedia/commons/thumb/2/24/Nahdlatul_Ulama_Logo.svg/512px-Nahdlatul_Ulama_Logo.svg.png', activeYear: new Date().getFullYear() };
        asalRanting = ""; 
...
// =====================================================================
// --- 6. FITUR PRO: TEMA GELAP, EKSPOR EXCEL, BACKUP JSON ---
// =====================================================================
...
function exportExcelBukuBesar() {
    if (typeof XLSX === 'undefined') {
        showToast("Pustaka Excel belum siap.", "error");
        return;
    }
    ...
    let balance = 0;
    const excelRows = sorted.map((t, index) => {
        const amt = Number(t.amount || 0);
        const isTransfer = (t.code === '120' || t.code === '220');
        
        if (t.type === 'in' && !isTransfer) balance += amt;
        if (t.type === 'out' && !isTransfer) balance -= amt;

        const catObj = allCats.find(c => c.code === t.code);
        const catName = catObj ? catObj.name : 'Lainnya';

        return {
            "No": index + 1,
            "Tanggal": t.date,
            "Akun": `[${t.code}] ${catName}`,
            "Sumber Dana": t.source || "Kas Tunai",
            "Uraian Transaksi": t.desc || "-",
            "Debit (Masuk)": t.type === 'in' ? amt : 0,
            "Kredit (Keluar)": t.type === 'out' ? amt : 0,
            "Saldo Kumulatif": balance
        };
    });

    const worksheet = XLSX.utils.json_to_sheet(excelRows);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Buku_Besar");
    
    const fileName = `Buku_Besar_UPZIS_${asalRanting || 'Karangdowo'}_${new Date().toISOString().slice(0, 10)}.xlsx`;
    XLSX.writeFile(workbook, fileName);
    showToast("Berkas Excel berhasil diunduh.", "success");
    saveLog('EKSPOR DATA', `Mengekspor Buku Besar ke Excel (${excelRows.length} baris)`);
}

function downloadJSONBackup() {
    const backupPayload = {
        meta: {
            aplikasi: "Sistem Bendahara Terpadu UPZIS",
            versi: "5.0 Pro",
            ranting: asalRanting || "Karangdowo",
    ...
    downloadAnchor.setAttribute("download", `Brankas_UPZIS_${asalRanting || 'Pusat'}_${Date.now()}.json`);
    document.body.appendChild(downloadAnchor);
    downloadAnchor.click();
    downloadAnchor.remove();

    showToast("File brankas JSON berhasil disimpan.", "success");
    saveLog('BACKUP JSON', 'Mengunduh arsip cadangan basis data JSON mandiri');
}

function restoreJSONBackup(event) {
    if (currentUserRole !== 'bendahara') {
        return alert("Otoritas terbatas. Hanya Bendahara yang diizinkan memulihkan data!");
    }
    ...
    reader.onload = async function(e) {
        try {
            ...
            if (!imported.transactions || !Array.isArray(imported.transactions)) {
                showToast("Format berkas JSON cadangan tidak valid.", "error");
                return;
            }

            const konfirmasi = confirm(`Ditemukan ${imported.transactions.length} riwayat transaksi dan ${imported.mustahik?.length || 0} data mustahik.\n\nApakah kamu yakin ingin mengimpor seluruh data ini ke Cloud?`);
            if (!konfirmasi) return;

            showToast("Memulihkan basis data ke Cloud...", "info");

            for (const item of imported.transactions) {
                const dataClean = { ...item };
                delete dataClean.idFirebase;
                delete dataClean.currentBalance;
                await window.fs.addDoc(getCol("transactions"), dataClean);
            }

            if (imported.mustahik && Array.isArray(imported.mustahik)) {
                for (const w of imported.mustahik) {
                    const wargaClean = { ...w };
                    delete wargaClean.idFirebase;
                    await window.fs.addDoc(getCol("mustahik"), wargaClean);
                }
            }

            if (imported.profile) {
                await window.fs.setDoc(getFirestoreDoc("settings", "profile"), imported.profile);
            }

            showToast("Pemulihan selesai! Seluruh data aktif kembali.", "success");
            saveLog('RESTORE JSON', `Memulihkan basis data dari ${file.name}`);
        } catch (err) {
            showToast("Gagal membaca struktur berkas JSON.", "error");
        }
    };
...
// =====================================================================
// --- 7. SINKRONISASI LAPORAN & REFRESH UI ---
// =====================================================================
...
function renderCharts() {
    ...
    const nuGreen = '#005a2b', dangerRed = '#ef4444';
...
// =====================================================================
// --- TRANSAKSI (CREATE, UPDATE, DELETE) ---
// =====================================================================
...
// =====================================================================
// --- FITUR TAMBAHAN (KALKULATOR, CETAK BUKU BESAR) ---
// =====================================================================
...
// =====================================================================
// --- PENGAMBILAN DATA (REAL-TIME LISTENER AUTO-SYNC) ---
// =====================================================================
...
// =====================================================================
// --- SOSIALISASI VIDEO (KOMPLEKS), MUSTAHIK, POSTER, CLOUD ---
// =====================================================================
function renderKontenHarian() {
    const container = document.getElementById('konten-harian-container');
    if(!container) return;

    container.innerHTML = `
    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:25px;">
        <div style="background:var(--bg-main); padding:24px; border-radius:16px; border:1px solid var(--border-color);">
            <h4 style="color:var(--nu-green-dark); margin-bottom:15px; font-size:14px;"><i class="fa-solid fa-pen-to-square"></i> Konfigurasi Pesan Kampanye</h4>
            
            <div class="form-group" style="margin-bottom:15px;">
                <label style="font-weight:800; font-size:11px; color:var(--text-muted); display:block; margin-bottom:6px;">Pilih Tema Konten Edukasi</label>
                <select id="kategori-kampanye" onchange="updateKampanyePreview()" style="width:100%; padding:12px; border-radius:10px; border:1px solid var(--border-color);">
                    <option value="sedekah">Keutamaan Sedekah (Al-Baqarah)</option>
                    <option value="zakat">Kewajiban Zakat (At-Taubah)</option>
                    <option value="motivasi">Motivasi Kebaikan (Hadis)</option>
                </select>
            </div>
            
            <div class="form-group" style="margin-bottom:20px;">
                <label style="font-weight:800; font-size:11px; color:var(--text-muted); display:block; margin-bottom:6px;">Pratinjau Teks (Bisa diedit manual)</label>
                <textarea id="teks-kampanye" rows="5" style="width:100%; padding:12px; border-radius:10px; border:1px solid var(--border-color); font-size:13px; line-height:1.6;"></textarea>
            </div>
            
            <div style="display:flex; gap:10px;">
                <button onclick="salinTeksKampanye()" class="btn btn-outline" style="flex:1;"><i class="fa-solid fa-copy"></i> Salin</button>
                <a id="btn-wa-kampanye" href="#" target="_blank" class="btn btn-primary" style="flex:2; background:var(--wa-color); border:none; color:white; text-decoration:none;"><i class="fa-brands fa-whatsapp"></i> Bagikan WA</a>
            </div>
        </div>

        <div style="background:var(--bg-main); padding:24px; border-radius:16px; border:1px solid var(--border-color); text-align:center;">
            <h4 style="color:var(--nu-green-dark); margin-bottom:15px; font-size:14px;"><i class="fa-solid fa-film"></i> Pratinjau Video Animasi AI</h4>
            <canvas id="video-canvas" width="800" height="800" style="width:100%; max-width:280px; border-radius:12px; box-shadow:var(--shadow-md); background:#012a14; margin-bottom:20px;"></canvas>
            <button id="btn-download-video" onclick="downloadCanvasAsVideo()" class="btn btn-primary" style="width:100%;"><i class="fa-solid fa-download"></i> Render & Unduh Video MP4</button>
        </div>
    </div>
    `;
    updateKampanyePreview();
}

function updateKampanyePreview() {
    const dbKonten = {
    ...
    const kat = document.getElementById('kategori-kampanye').value;
    const data = dbKonten[kat];
    window.activeKontenText = data; 
    
    const textLengkap = `*${data.judul}*\n\n"${data.teks}"\n\nMari sucikan harta dan salurkan ZIS Anda melalui UPZIS Ranting ${asalRanting ? asalRanting.toUpperCase() : 'KITA'}.\n\n_Sistem Bendahara Terpadu_`;
    
    document.getElementById('teks-kampanye').value = textLengkap;
    document.getElementById('btn-wa-kampanye').href = `https://api.whatsapp.com/send?text=${encodeURIComponent(textLengkap)}`;
    
    startVideoAnimation();
}

function salinTeksKampanye() {
    const teks = document.getElementById('teks-kampanye').value;
    navigator.clipboard.writeText(teks).then(() => { showToast("Teks berhasil disalin!", "success"); });
}
...
function startVideoAnimation() {
    ...
    logoImg.src = 'https://upload.wikimedia.org/wikipedia/commons/thumb/2/24/Nahdlatul_Ulama_Logo.svg/512px-Nahdlatul_Ulama_Logo.svg.png';
...
function downloadCanvasAsVideo() {
    ...
    mediaRecorder.start(); showToast("Merender Video Animasi... (Tunggu 6 detik)", "info"); const btn = document.getElementById('btn-download-video'); if(btn) { btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Rendering Video...'; btn.disabled = true; } setTimeout(() => { mediaRecorder.stop(); }, 6000);
}
...
window.updateKampanyePreview = updateKampanyePreview; window.salinTeksKampanye = salinTeksKampanye; window.downloadCanvasAsVideo = downloadCanvasAsVideo; 
window.importDataFromFile = importDataFromFile; window.syncCloudDrive = syncCloudDrive;
...
window.toggleThemeMode = toggleThemeMode; window.exportExcelBukuBesar = exportExcelBukuBesar; 
window.downloadJSONBackup = downloadJSONBackup; window.restoreJSONBackup = restoreJSONBackup;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.register('./sw.js?v=versi_pro_max')
...
// =====================================================================
// --- 13. SISTEM PEMANTAU EROR GLOBAL ---
// =====================================================================
...
window.addEventListener('unhandledrejection', function(event) {
    if (typeof showToast === "function") {
        console.warn("[Background Process]: Memproses di latar belakang.");
    }
});
