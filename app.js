...
window.openKuitansi = (id) => { 
    ...
    document.getElementById('kui-tipe').innerText = t.type === 'in' ? 'Donatur / Penerimaan' : 'Penyaluran / Mustahik';
    ...
    document.getElementById('kui-no').innerText = "TRX-" + (t.timestamp || Date.now()).toString().slice(-6);
    ...
    document.getElementById('kui-ttd').innerText = (profileSettings.anggota && profileSettings.anggota.find(a => a.jabatan.toLowerCase().includes('bendahara'))?.nama) || 'Bendahara Sistem';
...
window.eksekusiTutupBuku = async () => {
    ...
    if(confirm(`PERINGATAN! Anda yakin ingin menutup buku tahun ${year}?\nSemua transaksi akan dipindah ke Arsip dan Dashboard akan dikosongkan.`)) {
        try {
            // Proses pemindahan data ke koleksi arsip
...
// 5. Fungsi Batal Edit Transaksi (Mencegah ReferenceError)
...
// 2. Batal Edit Transaksi (Jika fitur edit dikembangkan)
...
// 3. Eksekusi Tutup Buku (Bahaya)
window.eksekusiTutupBuku = async function() {
    if(confirm("PERINGATAN! Ini akan menghapus semua data transaksi lama. Pastikan Anda sudah Backup JSON. Lanjutkan?")) {
        const pass = prompt("Ketik 'TUTUPBUKU' untuk konfirmasi:");
        if (pass === 'TUTUPBUKU') {
            showToast("Proses tutup buku belum diaktifkan di sisi server (Keamanan).", "info");
            // Logika penghapusan massal Firestore harusnya di sini
        } else {
            showToast("Batal. Kata sandi salah.", "error");
        }
    }
};

// 4. Fitur Hapus Mustahik
window.hapusMustahik = async function(id) {
    if(confirm("Hapus data warga ini?")) {
        await window.fs.deleteDoc(getFirestoreDoc("mustahik", id));
        showToast("Data Mustahik Dihapus", "success");
    }
};

// 5. Unduh Video Sosialisasi (Dummy respons agar tidak error)
window.downloadCanvasAsVideo = function() {
    showToast("Fitur konversi MP4 sedang diproses... Silakan gunakan screen record sementara.", "info");
};
