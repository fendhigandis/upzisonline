...
// centralized app logger (non-blocking)
window.appLog = async function(action, detail='', level='info'){
    try{
        if(level === 'error') console.error('[APP]', action, detail); else console.log('[APP]', action, detail);
        // non-blocking write to Firestore logs
        try{ await window.fs.addDoc(getCol('logs'), { time: new Date().toISOString(), action, detail: typeof detail === 'string' ? detail : JSON.stringify(detail), level }); }catch(e){}
    }catch(e){}
};
...
function getCol(col) { 
    try {
        if(!asalRanting || (typeof asalRanting === 'string' && asalRanting.toLowerCase() === "karangdowo")) return window.fs.collection(window.db, col);
...
function getFirestoreDoc(col, id) { 
    try {
        if(!asalRanting || (typeof asalRanting === 'string' && asalRanting.toLowerCase() === "karangdowo")) return window.fs.doc(window.db, col, id);
...
window.addEventListener('error', async (e) => { try{ await window.fs.addDoc(getCol("logs"), {time: new Date().toISOString(), action: 'SYSTEM_ERROR', detail: e.message}); }catch(err){console.error(err);} });
function applyRBAC() { document.querySelectorAll('.admin-only').forEach(elm => { elm.style.display = (currentUserRole === 'bendahara') ? '' : 'none'; }); }
...
async function kirimTelegram(jenis, nominal, desc) {
    ...
    try { await fetch(`https://api.telegram.org/bot${profileSettings.teleBotToken}/sendMessage?chat_id=${profileSettings.teleChatId}&text=${encodeURIComponent(pesan)}`); } catch(e){window.appLog('kirimTelegram failed', e, 'error');}
}
window.simpanTelegram = function() { try{ profileSettings.teleBotToken = el('tg-token')?.value || profileSettings.teleBotToken; profileSettings.teleChatId = el('tg-chatid')?.value || profileSettings.teleChatId; window.fs.setDoc(getFirestoreDoc("settings","profile"), profileSettings);}catch(e){window.appLog('simpanTelegram failed', e, 'error');} }

function refreshUI() {
    applyRBAC(); let grandIn = 0, grandOut = 0, saldoTunai = 0, saldoBank = 0, globalBalance = 0;
    const tbody = el('table-body'); if(tbody) tbody.innerHTML = '';
    ...
    const key = (el('search-keyword')?.value||'').toLowerCase();
    const st = el('filter-start-date')?.value||'', en = el('filter-end-date')?.value||'';
    const ft = el('filter-type')?.value||'';
    const minNom = parseFloat(el('filter-min')?.value) || 0;
    const maxNom = parseFloat(el('filter-max')?.value) || Infinity;
    ...
    if(tbody) {
        ...
        const pgIndicator = el('page-indicator'); if(pgIndicator) pgIndicator.innerText = `Hal ${currentPage} dari ${totalPages}`;
    }
    ['sum-tunai','sum-bank','sum-masuk','sum-keluar'].forEach((id,i) => { const element = el(id); if(element) element.innerText = formatRp([saldoTunai,saldoBank,grandIn,grandOut][i]); });
}

const amtInput = el('t-amount');
...
const formTrx = el('form-trx');
if(formTrx) formTrx.addEventListener('submit', async e => {
    e.preventDefault(); if(currentUserRole !== 'bendahara') return;
    const type = el('t-type')?.value, code = el('t-category')?.value, src = el('t-source')?.value;
    const date = el('t-date')?.value, desc = el('t-desc')?.value, amt = parseFloat((amtInput?.value||'').replace(/\./g,''));
    if(!amt || amt <= 0) return showToast("Nominal salah!", "error");
    const btnSubmit = el('btn-submit-trx'); if(btnSubmit) btnSubmit.disabled = true;
    try { await window.fs.addDoc(getCol("transactions"), { type, source:src, code, date, amount:amt, desc, timestamp: Date.now() });
        if(amtInput) amtInput.value=''; if(el('t-desc')) el('t-desc').value=''; showToast("Tersimpan!", "success"); kirimTelegram(type==='in'?'Masuk':'Keluar', amt, desc);
    } catch(err) { window.appLog('simpan transaksi failed', err, 'error'); showToast("Gagal", "error"); } finally { if(btnSubmit) btnSubmit.disabled = false; }
});
window.hapusTrx = async function(id) { if(confirm("Hapus?")) { try{ await window.fs.deleteDoc(getFirestoreDoc("transactions", id)); showToast("Dihapus", "success"); }catch(e){ window.appLog('hapusTrx failed', e, 'error'); showToast('Gagal Hapus','error'); } } }
...
window.buildReport = function() {
    let fStr = el('filter-month')?.value;
    if(!fStr) { fStr = new Date().toISOString().slice(0, 7); if(el('filter-month')) el('filter-month').value = fStr; }
    if(el('lap-periode')) el('lap-periode').innerText = `Bulan: ${fStr}`;
    ...
    if(el('lap-saldo-awal')) el('lap-saldo-awal').innerText = formatRp(sAw);
    if(el('lap-tot-masuk')) el('lap-tot-masuk').innerText = formatRp(tIn); 
    if(el('lap-tot-keluar')) el('lap-tot-keluar').innerText = formatRp(tOut); 
    if(el('lap-saldo-akhir')) el('lap-saldo-akhir').innerText = formatRp(sT+sB);
}
...
window.buildPoster = function() {
    ...
    if(el('poster-in')) el('poster-in').innerText = formatRp(tIn);
}

// SOSIALISASI AUTO-12 HOURS
function renderKontenHarian() {
    const c = el('konten-harian-container');
...
window.updateKampanyePreview = function() {
    ...
    if(el('teks-kampanye')) el('teks-kampanye').value = txt; 
    if(el('btn-wa-kampanye')) el('btn-wa-kampanye').href = `https://api.whatsapp.com/send?text=${encodeURIComponent(txt)}`;
    
    const cvs = el('video-canvas'); if(!cvs) return; const ctx = cvs.getContext('2d');
...
window.loadArsip = async function() {
    const year = el('input-arsip-year')?.value; if(!year) return; showToast(`Mencari arsip ${year}...`, "info");
    try { 
        ...
        const tbody = el('table-arsip-body'); if(tbody) tbody.innerHTML = html || '<tr><td colspan="5">Tidak ada arsip</td></tr>';
        if(!snap.empty) { const bar = el('arsip-action-bar'); if(bar) bar.classList.remove('hidden'); }
        window.currentArsipYear = year; window.currentArsipData = snap.docs.map(d => d.data());
        showToast("Arsip dimuat.", "success"); 
    } catch(e) { window.appLog('loadArsip failed', e, 'error'); showToast("Gagal memuat arsip.", "error"); }
}
...
window.cetakMustahikPDF = function() {
    const elPrint = el('mustahik-print-area'); if(!elPrint) return showToast('Tidak ada data untuk dicetak','error');
    html2pdf().set({ margin: 10, filename: `Data_Warga_${Date.now()}.pdf`, jsPDF: { format: 'a4', orientation: 'portrait' } }).from(elPrint).save();
}

// FITUR BARU: Email Kuitansi
window.kirimKuitansiEmail = function() {
    const nominal = el('kui-nominal')?.innerText, ket = el('kui-ket')?.innerText;
...
window.hubungkanGoogle = async function() {
    ...
    try {
        ...
        loadAllCloudData();
    } catch(e) { window.appLog('hubungkanGoogle failed', e, 'error'); showToast("Batal / Gagal menghubungkan Google.", "error"); }
}

// FITUR BARU: Ganti Sandi Butuh Sandi Lama
const formPass = el('form-password');
if(formPass) formPass.addEventListener('submit', async e => {
    ...
    try {
        ...
        window.appLog('change password failed', error, 'error');
...
window.simpanProfil = async function() { 
    ...
    profileSettings.lembaga = el('p-lembaga')?.value || profileSettings.lembaga; 
    profileSettings.periode = el('p-periode')?.value || profileSettings.periode; 
    const logoInput = el('p-logo')?.files && el('p-logo').files[0];
    if(logoInput) {
...
window.tambahAnggota = async function() { 
    const nama = el('input-anggota-nama')?.value.trim(), jabatan = el('input-anggota-jabatan')?.value.trim() || 'Relawan'; 
    const fotoInput = el('input-anggota-foto')?.files && el('input-anggota-foto').files[0];
    if(nama) { 
        ...
        const saveAnggota = async (fotoB64 = "") => {
            ...
            await window.fs.setDoc(getFirestoreDoc("settings", "profile"), profileSettings); 
            if(el('input-anggota-nama')) el('input-anggota-nama').value = '';
            if(el('input-anggota-foto')) el('input-anggota-foto').value = '';
...
function renderAnggota() { 
    ...
    const listElem = el('list-anggota'); if(!listElem) return;
...
function renderBagan() { 
    ...
    const c = el('org-chart-container'); if(c) c.innerHTML = html; 
}


// FITUR BARU: CRUD Inventaris
const formInv = el('form-inventaris');
if(formInv) formInv.addEventListener('submit', async e => {
    e.preventDefault(); if(currentUserRole !== 'bendahara') return;
    const item = { nama: el('i-nama')?.value, qty: el('i-qty')?.value, kondisi: el('i-kondisi')?.value };
...
function renderInventaris() {
    const tb = el('table-inventaris'); if(!tb) return;
...
const formAgenda = el('form-agenda');
if(formAgenda) formAgenda.addEventListener('submit', async e => {
    e.preventDefault(); if(currentUserRole !== 'bendahara') return;
    const ag = { tgl: el('a-tgl')?.value, judul: el('a-judul')?.value, hasil: el('a-hasil')?.value };
...
function renderAgenda() {
    const tb = el('table-agenda'); if(!tb) return;
...
const formMustahik = el('form-mustahik');
if(formMustahik) {
    formMustahik.addEventListener('submit', async e => {
        ...
        const kategori = el('m-kategori')?.value;
        ...
        try {
            ...
            window.appLog('add mustahik failed', error, 'error');
...
const formTodo = el('form-todo');
if(formTodo) formTodo.addEventListener('submit', async e => {
    e.preventDefault(); const task = el('td-task')?.value; if(!task) return;
...
function renderTodos() {
    const c = el('list-todo'); if(!c) return;
...
function renderLeaderboard() {
    const c = el('table-leaderboard'); if(!c) return;
    ...
    if(sorted.length > 0) {
        ...
        const elTop = el('leaderboard-top1'); if(elTop) elTop.innerText = (cat ? cat.name.replace('Setoran Bulanan dari Relawan ','') : 'Relawan') + ` (${formatRp(stats[topCode])})`;
    } else { const elTop = el('leaderboard-top1'); if(elTop) elTop.innerText = "Belum Ada Data Bulan Ini"; }
...
function renderLogs() { const tbody = el('table-logs'); if(!tbody) return; tbody.innerHTML = logsData.sort((a,b) => new Date(b.time) - new Date(a.time)).map(l => `<tr><td>${formatTanggalIndo(l.time)}</td><td><strong>${l.action}</strong></td><td><small>${l.detail||''}</small></td></tr>`).join(''); }

// =====================================================================
// --- 7. WINDOW BINDS UTAMA (TOMBOL KLIK HTML) ---
// =====================================================================
window.toggleSidebar = () => { const sb=el('app-sidebar'); const ov=el('sidebar-overlay'); if(sb) sb.classList.toggle('mobile-open'); if(ov) ov.classList.toggle('active'); }
window.toggleThemeMode = () => { document.body.classList.toggle('dark-mode'); }
window.updateCategories = () => { const t = el('t-type')?.value || 'in', c = getDynamicCategories(); const sel = el('t-category'); if(sel) sel.innerHTML = (c[t]||[]).map(x => `<option value="${x.code}">${x.code} - ${x.name}</option>`).join(''); }

// PENGHUBUNG TAB YANG MENJAMIN GRAFIK TAMPIL
window.switchTab = (e, tab) => { 
    document.querySelectorAll('.panel').forEach(p => p.classList.add('hidden')); 
    document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
    const panel = el(tab); if(panel) panel.classList.remove('hidden'); 
    if(e && e.currentTarget) e.currentTarget.classList.add('active');
    if(window.innerWidth <= 1024) { const sb=el('app-sidebar'); const ov=el('sidebar-overlay'); if(sb) sb.classList.remove('mobile-open'); if(ov) ov.classList.remove('active'); }
...
window.restoreJSONBackup = (e) => { const r = new FileReader(); r.onload = async ev => { try { const res = JSON.parse(ev.target.result); for(let t of res.transactions||[]){ delete t.idFirebase; await window.fs.addDoc(getCol('transactions'), t); } showToast('Backup dipulihkan', 'success'); } catch(err){ window.appLog('restore failed', err, 'error'); showToast('Gagal restore', 'error'); } }; if(e && e.target && e.target.files && e.target.files[0]) r.readAsText(e.target.files[0]); }
window.cetakStrukBluetooth = async () => { try { const device = await navigator.bluetooth.requestDevice({ filters: [{services: ['000018f0-0000-1000-8000-00805f9b34fb']}] }); await device.gatt.connect(); showToast('Terkoneksi ke printer (demo)', 'success'); } catch(e){ window.appLog('bluetooth print failed', e, 'error'); showToast('Printer Bluetooth tidak tersedia', 'error'); } }
window.cetakBukuBesarPDF = () => { const elPrint = el('buku-besar-area'); if(!elPrint) return showToast('Tidak ada data untuk dicetak','error'); html2pdf().set({ margin:10, filename:`BukuBesar_${Date.now()}.pdf`, jsPDF:{format:'a4', orientation:'landscape'} }).from(elPrint).save(); }
window.cetakPDF = () => { window.buildReport(); const elPrint = el('print-area'); if(!elPrint) return showToast('Tidak ada data untuk dicetak','error'); html2pdf().set({ margin:8, filename:`Laporan_${Date.now()}.pdf`, jsPDF:{format:'a4', orientation:'landscape'} }).from(elPrint).save(); }
window.cetakPosterPDF = () => { window.buildPoster(); const elPrint = el('poster-area'); if(!elPrint) return showToast('Tidak ada data untuk dicetak','error'); html2pdf().set({ margin:5, filename:`Poster_${Date.now()}.pdf`, jsPDF:{format:'a4', orientation:'portrait'} }).from(elPrint).save(); }
window.openKuitansi = (id) => { 
    const t = transactions.find(x => x.idFirebase === id); 
    if(!t) return;
    if(el('kui-nominal')) el('kui-nominal').innerText = formatRp(t.amount); 
    if(el('kui-tipe')) el('kui-tipe').innerText = t.type === 'in' ? 'Donatur / Penerimaan' : 'Penyaluran / Mustahik';
    if(el('kui-ket')) el('kui-ket').innerText = t.desc || '-';
    if(el('kui-sumber')) el('kui-sumber').innerText = t.source || '-';
    if(el('kui-tgl')) el('kui-tgl').innerText = formatTanggalIndo(t.date);
    if(el('kui-no')) el('kui-no').innerText = "TRX-" + (t.timestamp || Date.now()).toString().slice(-6);
    if(el('kui-lembaga')) el('kui-lembaga').innerText = profileSettings.lembaga || 'UPZIS';
    if(el('kui-periode')) el('kui-periode').innerText = profileSettings.periode || '';
    if(el('kui-ttd')) el('kui-ttd').innerText = (profileSettings.anggota && profileSettings.anggota.find(a => (a.jabatan||'').toLowerCase().includes('bendahara'))?.nama) || 'Bendahara Sistem';
    window.currentKuitansiData = t; // Simpan untuk PDF & WA
    const modal = el('modal-kuitansi'); if(modal) modal.classList.remove('hidden'); 
}

window.closeKuitansi = () => { const modal = el('modal-kuitansi'); if(modal) modal.classList.add('hidden'); };
window.bukaKalkulatorAmil = () => { const m = el('modal-amil'); if(m) m.classList.remove('hidden'); };

// 1. Fungsi Cetak PDF Kuitansi
window.cetakKuitansiPDF = () => {
    const elPrint = el('kuitansi-print-area'); if(!elPrint) return showToast('Tidak ada kuitansi','error');
    html2pdf().set({ margin: 5, filename: `Kuitansi_${Date.now()}.pdf`, jsPDF: { format: 'a5', orientation: 'landscape' } }).from(elPrint).save();
}
...
window.clearFilters = function() {
    if(el('search-keyword')) el('search-keyword').value = '';
    if(el('filter-type')) el('filter-type').value = '';
    if(el('filter-start-date')) el('filter-start-date').value = '';
    if(el('filter-end-date')) el('filter-end-date').value = '';
    if(el('filter-min')) el('filter-min').value = '';
    if(el('filter-max')) el('filter-max').value = '';
    currentPage = 1;
    refreshUI();
}
    ...
    if(confirm(`PERINGATAN! Anda yakin ingin menutup buku tahun ${year}?\nSemua transaksi akan dipindah ke Arsip dan Dashboard akan dikosongkan.`)) {
        try {
            for(let t of transactions) {
                ...
                try{ await window.fs.deleteDoc(getFirestoreDoc("transactions", t.idFirebase)); }catch(e){window.appLog('delete during tutup buku failed', e, 'error');}
            }
            showToast("Tutup buku berhasil! Data dipindah ke Arsip.", "success");
        } catch(e) {
            window.appLog('eksekusiTutupBuku failed', e, 'error');
...
window.cancelEditTrx = () => {
    const form = el('form-trx'); if(form) form.reset();
    const editId = el('t-edit-id'); if(editId) editId.value = '';
    const btnCancel = el('btn-cancel-edit'); if(btnCancel) btnCancel.classList.add('hidden');
    const btnSubmit = el('btn-submit-trx'); if(btnSubmit) btnSubmit.innerHTML = '<i class="fa-solid fa-check"></i> Rekam Transaksi';
}
...
async function loadAllCloudData() {
    try{
        ...
        window.fs.onSnapshot(getFirestoreDoc("settings", "profile"), (snap) => { if(snap && snap.exists()) { try{ profileSettings = {...profileSettings, ...snap.data()}; if(el('tg-token')) el('tg-token').value = profileSettings.teleBotToken || ''; if(el('tg-chatid')) el('tg-chatid').value = profileSettings.teleChatId || ''; renderAnggota(); }catch(e){window.appLog('profile snapshot handler failed', e, 'error');} } });
        // transactions
        ...
        window.fs.onSnapshot(getCol("mustahik"), (snap) => { dbMustahik=[]; snap.forEach(d => dbMustahik.push({idFirebase:d.id, ...d.data()})); const tb = el('table-mustahik'); if(tb) tb.innerHTML = dbMustahik.map(m=>`<tr><td>${m.nama}</td><td>${m.kategori}</td><td><button class="btn btn-outline" onclick="window.hapusMustahik('${m.idFirebase}')">Hapus</button></td></tr>`).join(''); });
    ...
    }catch(e){window.appLog('loadAllCloudData error', e, 'error');}    
}

window.togglePasswordVisibility = () => { const p=el('l-pass'), i=el('eye-icon'); if(p){ if(p.type==='password'){p.type='text'; if(i) i.className='fa-solid fa-eye-slash'; } else { p.type='password'; if(i) i.className='fa-solid fa-eye'; } } }
window.toggleModeAuth = (e) => { if(e) e.preventDefault(); isRegisterMode = !isRegisterMode; const grp=el('group-ranting'); if(grp) grp.style.display = isRegisterMode ? 'block' : 'none'; const btn=el('btn-auth-submit'); if(btn) btn.innerHTML = isRegisterMode ? '<i class="fa-solid fa-user-plus"></i> Daftar Baru' : '<i class="fa-solid fa-right-to-bracket"></i> Akses Sistem Terpadu'; }
window.recoverPasswordEmail = async () => { let em = el('l-user')?.value; if(!em) em = prompt("Masukkan Email Anda:"); if(!em) return; try{ await sendPasswordResetEmail(window.auth, em); showToast('Permintaan reset dikirim','success'); }catch(e){window.appLog('recoverPasswordEmail failed', e, 'error'); showToast('Gagal mengirim reset','error'); } }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function prosesLogin() {
        const email = el('l-user')?.value.trim();
        const pass = el('l-pass')?.value;
        const rantingName = el('l-ranting') ? el('l-ranting').value.trim() : "";
        const remember = el('remember-me') ? el('remember-me').checked : false;
        const btnSubmit = el('btn-auth-submit');
        ...
        try {
            ...
            window.appLog('prosesLogin failed', error, 'error');
    ...
    onAuthStateChanged(window.auth, async user => {
        if(user) {
            ...
            } catch(e){window.appLog('onAuthStateChanged user fetch failed', e, 'error');}
            const authScreen = el('auth-screen'); if(authScreen) authScreen.classList.add('hidden'); 
            const appScreen = el('app-screen'); if(appScreen) appScreen.classList.remove('hidden');
            loadAllCloudData(); applyRBAC();
        }
    });
});
// Mendaftarkan Service Worker (pada app.js saja)
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('./sw.js').then((registration) => {
            window.appLog('ServiceWorker registered', registration.scope);
            console.log('PWA ServiceWorker terdaftar dengan scope: ', registration.scope);
        }).catch((err) => {
            window.appLog('ServiceWorker register failed', err, 'error');
