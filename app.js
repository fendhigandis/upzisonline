...
let currentPage = 1; const itemsPerPage = 15; let totalPages = 1; let isRegisterMode = false; window.activeKontenText = null;
...
// =====================================================================
// --- 5. RENDER MENU TAB LAINNYA (GRAFIK, LAPORAN, POSTER, VIDEO) ---
// =====================================================================
function renderCharts() {
    ...
    const lm = sm.map(m => new Date(m.split('-')[0], m.split('-')[1]-1, 1).toLocaleString('id-ID', {month:'short', year:'numeric'}));
...
function renderKontenHarian() {
    ...
    if(!c || c.innerHTML.includes('select')) return; // Mencegah render ulang menumpuk
    c.innerHTML = `
    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:25px;">
        <div style="background:var(--bg-main); padding:24px; border-radius:16px; border:1px solid var(--border-color);">
            ...
            <div class="form-group"><label>Pilih Tema</label><select id="kategori-kampanye" onchange="window.updateKampanyePreview()"><option value="sedekah">Keutamaan Sedekah</option><option value="zakat">Kewajiban Zakat</option><option value="motivasi">Motivasi</option></select></div>
            <div class="form-group"><label>Pratinjau Teks</label><textarea id="teks-kampanye" rows="5"></textarea></div>
...
window.updateKampanyePreview = function() {
    ...
    const txt = `*${dbKonten[kat].judul}*\n\n"${dbKonten[kat].teks}"\n\nMari salurkan ZIS via UPZIS Ranting.`;
    ...
    ctx.fillStyle = '#012a14'; ctx.fillRect(0,0,800,800); ctx.fillStyle = '#d4af37'; ctx.font = 'bold 36px sans-serif'; ctx.textAlign = 'center'; ctx.fillText(dbKonten[kat].judul, 400, 400);
}

// =====================================================================
// --- 6. WINDOW BINDS & INIT MENGATUR LOGIN & TAB MENU ---
// =====================================================================
...
window.ubahHalaman = ubahHalaman; window.simpanTelegram = simpanTelegram; window.hapusTrx = hapusTrx;
window.exportExcelBukuBesar = () => showToast("Excel Diunduh", "success"); 
window.downloadJSONBackup = () => showToast("JSON Diunduh", "success"); 

// PERBAIKAN: Fungsi Tab Menu yang akan merender grafis jika halamannya dibuka
window.switchTab = (e, tab) => { 
    document.querySelectorAll('.panel').forEach(p => p.classList.add('hidden')); 
    document.getElementById(tab).classList.remove('hidden'); 
    
    if(tab === 'v-dashboard' || tab === 'v-histori') refreshUI();
    if(tab === 'v-laporan') buildReport();
    if(tab === 'v-poster') buildPoster();
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // PERBAIKAN: Fungsi Login Utuh Terkait dengan Tombol Daftar/Masuk
    ...
    if(btnAuth) { btnAuth.addEventListener('click', async e => { e.preventDefault(); await prosesLogin(); }); }

    // Memantau Pergantian Status Autentikasi Firebase
