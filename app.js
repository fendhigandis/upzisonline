...
// FITUR TAMBAHAN: Mengaktifkan Database Mode Offline (Penyimpanan Lokal)
...
document.getElementById('form-login').addEventListener('submit', async (e) => {
    ...
    try {
        showToast(isRegisterMode ? "Mendaftarkan ranting..." : "Memverifikasi...", "info");
        ...
        // --- PERBAIKAN DEBUGGING: Memunculkan pop-up error asli dari Firebase ---
        alert("INFO ERROR SISTEM:\nKode: " + error.code + "\nPesan: " + error.message);
        
        let errorMsg = "Akses Ditolak / Email salah.";
        if (error.code === 'auth/email-already-in-use') errorMsg = "Email sudah digunakan ranting lain!";
        else if (error.code === 'auth/too-many-requests') errorMsg = "Sistem diblokir sementara karena terlalu banyak percobaan.";
...
async function logout() {
    try {
        // PERBAIKAN: Menghentikan pemantauan Real-Time (mencegah kebocoran memori)
    ...
    const saveProcess = async (proofBase64 = null) => {
        try {
            ...
            if (editId) {
                ...
                await window.fs.updateDoc(getDoc("transactions", editId), updateData);
...
async function hapusTrx(idFirebase) { 
    ...
    if(confirm('Hapus permanen dari Cloud?')) { 
        try { await window.fs.deleteDoc(getDoc("transactions", idFirebase)); showToast("Transaksi dihapus.", "success"); saveLog('HAPUS TRANSAKSI', `Menghapus transaksi ID ${idFirebase}`); } catch (err) { showToast("Gagal menghapus.", "error"); }
    } 
}
...
async function loadAllCloudData() {
    try {
        // PERBAIKAN: Menyimpan fungsi onSnapshot ke dalam variabel agar bisa dihentikan
        unsubProfile = window.fs.onSnapshot(getDoc("settings", "profile"), (docSnap) => {
...
async function hapusMustahik(idFirebase) { 
    ...
    if(confirm('Hapus data warga ini dari Cloud?')) { 
        try { await window.fs.deleteDoc(getDoc("mustahik", idFirebase)); dbMustahik = dbMustahik.filter(m => m.idFirebase !== idFirebase); renderMustahik(); updateStorageIndicator(); saveLog('HAPUS WARGA', `Menghapus data mustahik ID ${idFirebase}`); showToast("Data dihapus.", "success"); } catch (err) { showToast("Gagal menghapus data warga.", "error"); }
    } 
}
...
// PERBAIKAN: Menghitung Sisa Saldo saat Tutup Buku
async function wipeAllDataForNewYear(newYear) {
    ...
    try {
        ...
        trxSnap.forEach(d => { deletePromises.push(window.fs.setDoc(getDoc(`arsip_${activeYear}_transactions`, d.id), d.data())); deletePromises.push(window.fs.deleteDoc(getDoc("transactions", d.id))); }); await Promise.all(deletePromises);
        profileSettings.activeYear = newYear; await window.fs.setDoc(getDoc("settings", "profile"), profileSettings);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Mendaftarkan Service Worker (PWA) agar bisa diinstal
    ...
    // -----------------------------------------------------------------
    // --- TAMBAHAN FIX 405: Mencegah Form Pengaturan Default Submit ---
    // -----------------------------------------------------------------
...
// =====================================================================
// Menangkap error logika atau kerusakan file di browser
window.addEventListener('error', function(event) {
    console.error("[Sistem Error]: ", event.message);
    // Munculkan notifikasi jika fungsi showToast sudah tersedia di memori
    if (typeof showToast === "function") {
        showToast("Terjadi gangguan pada sistem perangkat. Coba muat ulang (Refresh) halaman.", "error");
    }
});

// Menangkap error kegagalan janji/koneksi (Promise Rejection) yang tidak tertangani
window.addEventListener('unhandledrejection', function(event) {
    console.error("[Koneksi/Promise Error]: ", event.reason);
    if (typeof showToast === "function") {
