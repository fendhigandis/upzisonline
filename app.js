...
window.authServices = { signInWithEmailAndPassword, signOut, onAuthStateChanged, setPersistence, browserLocalPersistence, browserSessionPersistence, createUserWithEmailAndPassword, sendPasswordResetEmail, GoogleAuthProvider, linkWithPopup, signInWithPopup, EmailAuthProvider, reauthenticateWithCredential, updatePassword };
...
function refreshUI() {
    ...
    renderRincianRelawanDashboard();
}

// FITUR: Pastikan SEMUA relawan terdaftar tampil di Dashboard (termasuk yang belum setor bulan ini / Rp 0)
function renderRincianRelawanDashboard() {
    const c = document.getElementById('dash-rincian-relawan'); if(!c) return;
    const relawanList = profileSettings.relawan || [];
    if(relawanList.length === 0) { c.innerHTML = '<div style="font-size:12px; color:var(--text-muted); grid-column:1/-1;">Belum ada relawan terdaftar. Tambahkan di menu Pengurus.</div>'; return; }
    const currentM = new Date().toISOString().slice(0,7);
    const cats = getDynamicCategories().in;
    c.innerHTML = relawanList.map((nama, i) => {
        const code = String(111 + i);
        const total = transactions.filter(t => t.type === 'in' && t.code === code && (t.date||'').slice(0,7) === currentM).reduce((a,t) => a + Number(t.amount||0), 0);
        return `<div class="mini-relawan-card" style="border:1px solid var(--border-color); border-radius:10px; padding:10px 14px;"><div style="font-size:11px; color:var(--text-muted);">${nama}</div><div style="font-weight:800; color:${total>0?'#15803d':'#94a3b8'};">${formatRp(total)}</div></div>`;
    }).join('');
}
...
function renderKontenHarian() {
    ...
    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:25px;">
        ...
        <div style="background:linear-gradient(135deg, var(--nu-green-dark), #012a14); color:white; padding:24px; border-radius:16px; text-align:center;">
            <h4 style="margin-bottom:5px; font-size:14px; color:var(--nu-gold);"><i class="fa-solid fa-mosque"></i> Jam Digital &amp; Waktu Sholat</h4>
            <div id="jam-digital" style="font-size:34px; font-weight:800; font-family:'Cinzel'; margin:10px 0 2px;">--:--:--</div>
            <div id="tanggal-digital" style="font-size:12px; opacity:0.8; margin-bottom:14px;">-</div>
            <div id="waktu-sholat-list" style="display:grid; grid-template-columns: repeat(5, 1fr); gap:6px; margin-bottom:12px;"></div>
            <div id="sholat-berikutnya" style="font-size:12px; background:rgba(255,255,255,0.1); padding:8px; border-radius:8px; margin-bottom:12px;">Memuat jadwal sholat...</div>
            <div style="display:flex; align-items:center; justify-content:center; gap:8px; font-size:11px;"><label style="display:flex; align-items:center; gap:6px; cursor:pointer;"><input type="checkbox" id="chk-adzan-otomatis" checked onchange="window.toggleAdzanOtomatis(this.checked)"> Suara Adzan Otomatis</label> <button class="btn-action" style="color:var(--nu-gold); font-size:16px;" onclick="window.testSuaraAdzan()" title="Tes Suara Adzan"><i class="fa-solid fa-volume-high"></i></button></div>
        </div>
    </div>`;
    window.updateKampanyePreview();
    initJamDigitalDanSholat();
}
...
window.updateKampanyePreview = function() {
    const templates = [
        ...
        { judul: "Tidak Mengurangi Harta", teks: "Sedekah tidak akan mengurangi harta. (HR. Muslim)" },
        { judul: "Naungan di Hari Kiamat", teks: "Setiap orang berada dalam naungan sedekahnya hingga diputuskan perkara di antara manusia. (HR. Ahmad)" },
        { judul: "Zakat Penyempurna Iman", teks: "Dan dirikanlah salat, tunaikanlah zakat, dan rukuklah beserta orang-orang yang rukuk. (Al-Baqarah: 43)" },
        { judul: "Delapan Golongan Mustahik", teks: "Sesungguhnya zakat itu untuk fakir miskin, amil, muallaf, riqab, gharim, fisabilillah, dan ibnu sabil. (At-Taubah: 60)" },
        { judul: "Tangan di Atas Lebih Baik", teks: "Tangan yang di atas lebih baik daripada tangan yang di bawah. (HR. Bukhari)" },
        { judul: "Sedekah Sembunyi", teks: "Sedekah yang paling utama adalah sedekah yang dirahasiakan kepada orang fakir. (HR. Ahmad)" },
        { judul: "Amalan Tak Terputus", teks: "Apabila manusia meninggal dunia, terputuslah amalnya kecuali tiga perkara, salah satunya sedekah jariyah. (HR. Muslim)" },
        { judul: "Menutup Pintu Keburukan", teks: "Sedekah dapat memadamkan kesalahan sebagaimana air memadamkan api. (HR. Tirmidzi)" },
        { judul: "Balasan Berlipat Ganda", teks: "Barang siapa membawa amal kebaikan, maka baginya sepuluh kali lipat amalnya. (Al-An'am: 160)" },
        { judul: "Infak di Waktu Lapang dan Sempit", teks: "Orang-orang yang menginfakkan hartanya baik di waktu lapang maupun sempit. (Ali Imran: 134)" },
        { judul: "Menyambung Silaturahmi", teks: "Sedekah kepada orang miskin bernilai sedekah, dan kepada kerabat bernilai sedekah dan silaturahmi. (HR. Nasa'i & Tirmidzi)" },
        { judul: "Doa Malaikat untuk Dermawan", teks: "Ya Allah, berilah ganti bagi orang yang berinfak. (HR. Bukhari & Muslim)" }
    ];
...
window.hubungkanGoogle = async function() {
    if(!window.auth.currentUser) return showToast("Silakan login terlebih dahulu sebelum menghubungkan Google.", "error");
    ...
    try {
        ...
        if(e.code === 'auth/credential-already-in-use' || e.code === 'auth/email-already-in-use') {
            showToast("Akun Google ini sudah terdaftar terpisah. Mencoba masuk langsung...", "info");
            try { await window.authServices.signInWithPopup(window.auth, provider); showToast("Berhasil masuk dengan Akun Google!", "success"); loadAllCloudData(); }
            catch(e2) { showToast(`Gagal masuk dengan Google: ${e2.code || e2.message}`, "error"); }
        } else if(e.code === 'auth/popup-blocked') {
            showToast("Popup diblokir browser. Izinkan pop-up untuk situs ini lalu coba lagi.", "error");
        } else if(e.code === 'auth/unauthorized-domain') {
            showToast("Domain ini belum diizinkan di Firebase Console (Authentication > Settings > Authorized domains).", "error");
        } else if(e.code === 'auth/operation-not-allowed') {
            showToast("Metode masuk Google belum diaktifkan di Firebase Console (Authentication > Sign-in method).", "error");
        } else if(e.code !== 'auth/popup-closed-by-user' && e.code !== 'auth/cancelled-popup-request') {
            showToast(`Gagal menghubungkan Google: ${e.code || e.message}`, "error");
        }
    }
}
...
function renderBagan() {
    const list = profileSettings.anggota || [];
    const pimpinan = list.filter(a => (a.jabatan || '').toLowerCase().includes('ketua'));
    const bendahara = list.filter(a => (a.jabatan || '').toLowerCase().includes('bendahara'));
    const sekretaris = list.filter(a => (a.jabatan || '').toLowerCase().includes('sekretaris'));
    const relawan = list.filter(a => !(a.jabatan || '').toLowerCase().includes('ketua') && !(a.jabatan || '').toLowerCase().includes('bendahara') && !(a.jabatan || '').toLowerCase().includes('sekretaris'));
    const makeCard = (a, tier) => `<div class="org-card org-tier-${tier}">${a.foto ? `<img src="${a.foto}" class="org-card-photo">` : `<div class="org-card-photo org-card-icon"><i class="fa-solid fa-user-tie"></i></div>`}<div class="org-card-jabatan">${a.jabatan}</div><div class="org-card-nama">${a.nama}</div></div>`;
    const c = document.getElementById('org-chart-container'); if(!c) return;
    if(list.length === 0) { c.innerHTML = '<div style="text-align:center; padding:30px; color:var(--text-muted); font-size:13px;">Belum ada data pengurus. Tambahkan melalui form di atas.</div>'; return; }
    let html = '<div class="org-chart-tree">';
    if(pimpinan.length) html += `<div class="org-level"><div class="org-row">${pimpinan.map(a=>makeCard(a,1)).join('')}</div></div>`;
    if(sekretaris.length || bendahara.length) {
        html += `<div class="org-connector"></div><div class="org-level"><div class="org-row">${sekretaris.map(a=>makeCard(a,2)).join('')}${bendahara.map(a=>makeCard(a,2)).join('')}</div></div>`;
    }
    if(relawan.length) {
        html += `<div class="org-connector"></div><div class="org-level"><div class="org-section-label">Relawan &amp; Kolektor ZIS</div><div class="org-row org-row-wrap">${relawan.map(a=>makeCard(a,3)).join('')}</div></div>`;
    }
    html += '</div>';
    c.innerHTML = html;
}
...
function renderLeaderboard() {
    ...
    if(sorted.length > 0) {
        ...
        document.getElementById('leaderboard-top1').innerText = (cat ? cat.name.replace('Setoran Bulanan dari Relawan ','') : 'Relawan') + ` (${formatRp(stats[topCode])})`;
...
window.exportExcelBukuBesar = () => { if(typeof XLSX === 'undefined') return showToast("Pustaka Excel gagal dimuat. Periksa koneksi internet.", "error"); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(transactions.map(t=>({"Tanggal":t.date,"Akun":t.code,"Sumber":t.source,"Debit":t.type==='in'?t.amount:0,"Kredit":t.type==='out'?t.amount:0,"Ket":t.desc}))), "Buku_Besar"); XLSX.writeFile(wb, `Buku_Besar_${Date.now()}.xlsx`); showToast("Excel berhasil diunduh.", "success"); }
function withExportMode(fn) {
    if(typeof html2pdf === 'undefined') { showToast("Gagal memuat pustaka PDF (html2pdf). Periksa koneksi internet Anda, lalu muat ulang halaman.", "error"); return Promise.resolve(); }
    document.body.classList.add('exporting-mode');
    const cleanup = (ok) => { document.body.classList.remove('exporting-mode'); if(ok) showToast("PDF berhasil diunduh.", "success"); else showToast("Gagal membuat PDF. Coba muat ulang halaman.", "error"); };
    try { return Promise.resolve(fn()).then(() => cleanup(true), (err) => { console.error(err); cleanup(false); }); } catch(e) { console.error(e); cleanup(false); }
}
...
// PENTING: app.js dimuat sebagai ES module, sehingga fungsi biasa (function/const di atas)
// TIDAK otomatis tersedia untuk atribut inline HTML (onclick/onkeyup/onchange) yang berjalan
// di scope global. Ini akar penyebab Kalkulator Zakat & kolom pencarian/filter tidak berfungsi.
window.formatRp = formatRp;
window.refreshUI = refreshUI;
window.loadAllCloudData = loadAllCloudData;
...
window.exportFullExcel = function() {
    if(typeof XLSX === 'undefined') return showToast("Pustaka Excel gagal dimuat. Periksa koneksi internet.", "error");
...
// =====================================================================
// --- 17. JAM REAL-TIME, JADWAL SHOLAT & SUARA ADZAN OTOMATIS ---
// =====================================================================
let waktuSholatData = null; let waktuSholatTanggal = null; let adzanTerakhirDipicu = null; let adzanEnabled = true;
window.toggleAdzanOtomatis = (v) => { adzanEnabled = v; }
window.testSuaraAdzan = function() {
    const a = document.getElementById('audio-adzan'); if(!a) return;
    a.currentTime = 0; a.play().then(()=>showToast("Memutar suara adzan (tes)...", "success")).catch(()=>showToast("Gagal memutar audio. Klik dulu di halaman lalu coba lagi.", "error"));
}
// Browser memblokir autoplay audio tanpa interaksi pengguna. Buka "kunci" audio pada interaksi pertama
// agar saat waktu adzan tiba nanti, audio bisa diputar otomatis tanpa perlu klik lagi.
function unlockAdzanAudioOnce() {
    const a = document.getElementById('audio-adzan');
    if(a) { a.muted = true; a.play().then(() => { a.pause(); a.currentTime = 0; a.muted = false; }).catch(() => {}); }
    document.removeEventListener('click', unlockAdzanAudioOnce);
}
document.addEventListener('click', unlockAdzanAudioOnce, { once: true });

async function ambilJadwalSholat(lat, lon) {
    try {
        const ts = Math.floor(Date.now() / 1000);
        const res = await fetch(`https://api.aladhan.com/v1/timings/${ts}?latitude=${lat}&longitude=${lon}&method=20`);
        const json = await res.json();
        if(json && json.data && json.data.timings) { waktuSholatData = json.data.timings; waktuSholatTanggal = new Date().toDateString(); renderWaktuSholat(); }
    } catch(err) { const el = document.getElementById('sholat-berikutnya'); if(el) el.innerText = "Gagal memuat jadwal sholat (periksa koneksi internet)."; }
}
function siapkanJadwalSholat() {
    // Cache per hari agar tidak memanggil API berulang kali setiap kali tab dibuka
    if(waktuSholatData && waktuSholatTanggal === new Date().toDateString()) return renderWaktuSholat();
    const fallback = () => ambilJadwalSholat(-7.68, 110.69); // Perkiraan lokasi default (Karangdowo, Klaten) jika lokasi tak tersedia
    if(navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            pos => ambilJadwalSholat(pos.coords.latitude, pos.coords.longitude),
            () => fallback(),
            { timeout: 6000 }
        );
    } else { fallback(); }
}
function renderWaktuSholat() {
    const list = document.getElementById('waktu-sholat-list'); if(!list || !waktuSholatData) return;
    const map = [['Subuh','Fajr'],['Dzuhur','Dhuhr'],['Ashar','Asr'],['Maghrib','Maghrib'],['Isya','Isha']];
    list.innerHTML = map.map(([label,key]) => `<div style="background:rgba(255,255,255,0.1); border-radius:8px; padding:6px 2px;"><div style="font-size:9px; opacity:0.75;">${label}</div><div style="font-size:13px; font-weight:800;">${(waktuSholatData[key]||'-').slice(0,5)}</div></div>`).join('');
}
function cekPemicuAdzan() {
    if(!waktuSholatData || !adzanEnabled) return;
    const now = new Date(); const hhmm = now.toTimeString().slice(0,5);
    const namaMap = { Fajr:'Subuh', Dhuhr:'Dzuhur', Asr:'Ashar', Maghrib:'Maghrib', Isha:'Isya' };
    for(const key of Object.keys(namaMap)) {
        const t = (waktuSholatData[key]||'').slice(0,5);
        const triggerId = `${now.toDateString()}_${key}`;
        if(t === hhmm && adzanTerakhirDipicu !== triggerId) {
            adzanTerakhirDipicu = triggerId;
            const a = document.getElementById('audio-adzan');
            if(a) a.play().catch(()=>{});
            showToast(`Telah masuk waktu ${namaMap[key]}. Suara adzan diputar otomatis.`, "success");
        }
    }
}
function tampilkanCountdownSholat() {
    const el = document.getElementById('sholat-berikutnya'); if(!el || !waktuSholatData) return;
    const now = new Date(); const order = [['Subuh','Fajr'],['Dzuhur','Dhuhr'],['Ashar','Asr'],['Maghrib','Maghrib'],['Isya','Isha']];
    let next = null;
    for(const [label,key] of order) {
        const t = (waktuSholatData[key]||'').slice(0,5); if(!t) continue;
        const [h,m] = t.split(':').map(Number); const target = new Date(now); target.setHours(h,m,0,0);
        if(target > now) { next = { label, target }; break; }
    }
    if(!next) { const [h,m] = (waktuSholatData.Fajr||'04:00').slice(0,5).split(':').map(Number); const target = new Date(now); target.setDate(target.getDate()+1); target.setHours(h,m,0,0); next = { label:'Subuh', target }; }
    const diff = next.target - now; const hh = String(Math.floor(diff/3600000)).padStart(2,'0'), mm = String(Math.floor((diff%3600000)/60000)).padStart(2,'0'), ss = String(Math.floor((diff%60000)/1000)).padStart(2,'0');
    el.innerHTML = `Menuju waktu <strong>${next.label}</strong>: ${hh}:${mm}:${ss}`;
}
function initJamDigitalDanSholat() {
    if(window._jamDigitalInterval) clearInterval(window._jamDigitalInterval);
    siapkanJadwalSholat();
    const tick = () => {
        const now = new Date();
        const jamEl = document.getElementById('jam-digital'), tglEl = document.getElementById('tanggal-digital');
        if(jamEl) jamEl.innerText = now.toLocaleTimeString('id-ID');
        if(tglEl) tglEl.innerText = now.toLocaleDateString('id-ID', { weekday:'long', day:'numeric', month:'long', year:'numeric' });
        tampilkanCountdownSholat(); cekPemicuAdzan();
    };
    tick(); window._jamDigitalInterval = setInterval(tick, 1000);
}
