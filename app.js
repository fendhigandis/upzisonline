...
window.toggleModeAuth = (e) => {
    e.preventDefault(); isRegisterMode = !isRegisterMode;
    document.getElementById('group-ranting').style.display = isRegisterMode ? 'block' : 'none';
    document.getElementById('l-ranting').required = isRegisterMode;
    document.getElementById('btn-auth-submit').innerHTML = isRegisterMode ? '<i class="fa-solid fa-user-plus"></i> Daftar Baru' : '<i class="fa-solid fa-right-to-bracket"></i> Akses Sistem';
    document.getElementById('link-toggle-auth').innerText = isRegisterMode ? 'Sudah punya akun? Masuk' : 'Belum punya akun? Daftar';
};
window.recoverPasswordEmail = async () => {
    let em = document.getElementById('l-user').value; if(!em) em = prompt("Masukkan Email Anda:"); if(!em) return;
    try{ await window.authServices.sendPasswordResetEmail(window.auth, em); showToast("Link pemulihan dikirim!", "success"); }catch(e){ showToast("Gagal kirim pemulihan", "error"); }
};
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // PERBAIKAN: Fungsi proses login yang utuh dipulihkan kembali
    async function prosesLogin() {
        ...
        try {
            ...
            showToast("Sandi salah atau periksa koneksi.", "error");
        } finally {
            if(btnSubmit) btnSubmit.innerHTML = isRegisterMode ? '<i class="fa-solid fa-user-plus"></i> Daftar Baru' : '<i class="fa-solid fa-right-to-bracket"></i> Akses Sistem';
        }
    }
    ...
    document.getElementById('form-login').addEventListener('submit', async e => {
        e.preventDefault(); await prosesLogin();
    });
